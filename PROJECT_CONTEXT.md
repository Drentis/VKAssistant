...
2. **Напоминания не работают** — планировщик не реализован в текущей версии
   - Нужно добавить `reminder_scheduler()` как в TelegramAssistant

3. **Рецепты без FSM** — добавление ингредиентов работает, но без состояния
   - Нужно реализовать полноценный FSM для VK
